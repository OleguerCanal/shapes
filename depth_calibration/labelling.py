...
class Labeller():
    ...
    def get_gradient_matrices(self, center, radius, size=(480, 640)):
        ...
        for x_pixel in range(n_x):
            for y_pixel in range(n_y):
                if self.__distance((x_pixel, y_pixel), center) < pixel_r:
    ...
    def labell_image(self, img, img_back):
        ...
        print center, radius
        dz_dx_mat, dz_dy_mat = self.get_gradient_matrices(center=center, radius=radius)
        return dz_dx_mat, dz_dy_mat
